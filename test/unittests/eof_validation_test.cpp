...
TEST_F(eof_validation, EOF1_create3_valid)
{
...
TEST_F(eof_validation, EOF1_create3_invalid)
{
    // truncated immediate
    const auto embedded = eof_bytecode(bytecode{OP_INVALID});
    add_test_case(eof_bytecode(bytecode(0) + 0xff + 0 + 0 + OP_CREATE3, 4).container(embedded),
        EOFValidationError::truncated_instruction);

    // last instruction
    add_test_case(
        eof_bytecode(bytecode(0) + 0xff + 0 + 0 + OP_CREATE3 + Opcode{0}, 4).container(embedded),
        EOFValidationError::no_terminating_instruction);

    // referring to non-existent container section
    add_test_case(
        eof_bytecode(bytecode(0) + 0xff + 0 + 0 + OP_CREATE3 + Opcode{1} + OP_POP + OP_STOP, 4)
            .container(embedded),
    ...
    add_test_case(
        eof_bytecode(bytecode(0) + 0xff + 0 + 0 + OP_CREATE3 + Opcode{0} + OP_POP + OP_STOP, 4)
            .container(embedded_truncated_data),
        EOFValidationError::create3_with_truncated_container);
}
